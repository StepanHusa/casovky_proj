...
Na základě předchozí analýzy ACF a PACF a dekompozice řady přistupujeme k manuální konstrukci modelu. Nejprve však data logaritmujeme, abychom stabilizovali rostoucí rozptyl (heteroskedasticitu), který jsme pozorovali v grafické analýze.Fragment kódu
...
-   **Trend (**$d=1$):
    Původní časová řada vykazovala jasný rostoucí trend a nestacionaritu. Aplikace první diference vedla k odstranění nestacionarity, což potvrdil Augmented Dickey-Fullerův test (p-hodnota $< 0.01$).
    Navíc graf ACF diferencované řady ukazuje na lagu 1 a 2 významné záporné hodnoty. Tento jev je v literatuře považován za varovný signál proti dalšímu diferencování (riziko tzv. *over-differencing*). Na základě těchto statistických i vizuálních testů proto fixujeme řád diferenciace na **\$d=1\$**.

-   **Sezónnost (**$D=1, s=12$):
    Časová řada vykazuje zřetelnou roční cykličnost. Klíčovým indikátorem je statisticky významná kladná hodnota autokorelační funkce (ACF) na lagu 12, která přetrvává i po provedení první diference. Tento vzor indikuje přítomnost tzv. stochastické sezónnosti, kterou je nutné modelovat aplikací sezónní diference ($D=1$) s periodou $s=12$.
...
lb_test_manual_final <- checkresiduals(fit_manual_final, plot = FALSE, lag = 24)
print(lb_test_manual_final)
```
...
Pokud auto.arima zvolila stejný model nebo model s velmi podobným AICc, potvrzuje to kvalitu naší manuální identifikace. Za finální model pro predikci volíme ten s nižším AICc (pravděpodobně náš redukovaný model nebo model shodný).
...
# Ljung-Boxův test nezávislosti reziduí
# H0: Rezidua jsou nezávislá (model je v pořádku)
# H1: Rezidua jsou autokorelovaná
lb_test <- checkresiduals(final_model, plot = FALSE, lag = 24)
print(lb_test)
```

Vyhodnocení diagnostiky:Pokud je p-hodnota Ljung-Boxova testu $> 0.05$, nezamítáme nulovou hypotézu. To znamená, že model úspěšně zachytil veškerou strukturu v datech a rezidua jsou náhodná.7. PredikceProtože model vyhovuje diagnostickým testům, provedeme predikci na 12 měsíců dopředu. Je nutné provést zpětnou transformaci (exponenciálu), protože model pracoval s logaritmy.
...
V této práci jsme analyzovali vývoj popularity jazyka Python na StackOverflow.Zjistili jsme silný rostoucí trend a sezónnost.Pomocí logaritmické transformace a diferencování jsme data stacionarizovali.Manuální analýzou koeficientů a srovnáním s automatickou selekcí jsme identifikovali jako nejvhodnější model SARIMA(1,1,1)(0,1,1)\[12\].Tento model prošel diagnostickými testy a byl použit k predikci, která očekává další růst zájmu s typickými sezónními výkyvy.
